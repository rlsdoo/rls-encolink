/**
 * @file    write_read_register.ino
 * @brief   Write `POS_OFFSET` and read it back while position streaming continues.
 * @details Sequence:
 *          1) Initialize SPI and EncoLink.
 *          2) Repeatedly call read_encoder() for cyclic position updates.
 *          3) Trigger write_register_command(POS_OFFSET, value).
 *          4) Poll get_write_register_status() until completion.
 *          5) Trigger read_register_command(POS_OFFSET) and verify the value.
...
void setup()
{
	...
	// Read identification number from encoder
...
{							
	...
	while (1)
	{
		...
		{
			ra_buffer = enco.write_register_command(POS_OFFSET,register_value);
