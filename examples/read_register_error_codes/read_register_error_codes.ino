/**
 * @file    read_register_error_codes.ino
 * @brief   Demonstrate register-read error handling with invalid read requests.
 * @details Sequence:
 *          1) Initialize SPI and EncoLink.
 *          2) Repeatedly call read_encoder() for cyclic position updates.
 *          3) Trigger a read command with invalid parameters (scenario-based).
 *          4) Poll get_read_register_data() for completion status.
 *          5) Print returned error/status code for diagnostics.
...
void read_register_command(uint8_t scenario)
{
	switch (scenario)
	{
	// 1. Entering wrong number of bytes (Temperature register)
	case 0: 
		ra_buffer = enco.read_register_command(1, 0x00); // (number_of_bytes, address)
		break;
	// 2. Entering invalid parameter id/register address
	case 1:
		ra_buffer = enco.read_register_command(4, 0x14);
		break;
	}
}
